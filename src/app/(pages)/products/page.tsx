...
const page = async ({
  ...
  const products = subCategory
    ...
    : brand
      ? await getProducts(undefined, brand)
      : await getProducts();
            ...
            <p className="text-sm leading-5 ">
              {subCategory || brand ? (
                <>
                  ...
                  <span>{products?.[0]?.subcategory?.[0]?.name}</span>
                </>
              ) : (
                <>
                  <Link href={"/"} className="opacity-70 hover:opacity-100">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-4xl leading-10 font-bold ">
                  {subCategory || brand
                    ? (products?.[0]?.subcategory?.[0]?.name ?? "Products")
                    : "All Products"}
                </h1>
                <p className="opacity-80">
                  {subCategory || brand
                    ? `Browse ${products?.[0]?.subcategory?.[0]?.name ?? ""} products`
